...
func setupTest() (*gin.Engine, *repositories.FavoriteRepoMock, *FavoriteHandlers) {
...
func TestAddFavoriteHandler(t *testing.T) {
	r, favoriteRepositoryMock, handlers := setupTest()
...
func TestGetFavoritesHandler(t *testing.T) {
	...
	assert.Equal(t, "Chicken Sandwich", actualData[0].(map[string]interface{})["product_name"])
	assert.Equal(t, "Kentang Goreng", actualData[1].(map[string]interface{})["product_name"])
